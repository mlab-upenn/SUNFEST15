...
UsedX = college_hall_tree14.X;
loss = length(UsedX) ./ length(hum);
UsedY = college_hall_tree14.Y;
lossy = length(UsedY) ./ length(Y);
...
title('Prediction vs Ground Truth','FontSize',22);
xlabel('Time (Hours)','FontSize',26)
...
[fX,fY,len,loss] = newCleanXY(X,Y,sigmah,sigmal);

% interpolates over 0s
[iX,iY] = InterPenn (fX,fY);
